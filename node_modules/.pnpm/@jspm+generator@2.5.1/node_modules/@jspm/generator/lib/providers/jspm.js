...
export async function pkgToUrl(pkg, layer) {
...
async function ensureBuild(resolver, pkg, fetchOpts) {
    if (await checkBuildOrError(resolver, await pkgToUrl(pkg, "default"), fetchOpts)) return;
...
export async function fetchVersions(name) {
    ...
    const registryLookup = JSON.parse(await await fetch.text(`https://npmlookup.jspm.io/${encodeURI(name)}`, {})) || {};
