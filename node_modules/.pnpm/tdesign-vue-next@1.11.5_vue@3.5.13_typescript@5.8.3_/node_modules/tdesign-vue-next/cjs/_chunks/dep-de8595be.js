...
function isInputNumber(num) {
  if (!num) return true;
  if (isNumber.isNumber(num)) return !Number.isNaN(num);
...
function removeInvalidZero(num) {
  ...
  if (num.indexOf(".") !== -1) {
    log.log.error("InputNumber", "num is not a integer number.");
    return num;
  }
...
function formatDecimal(num, places) {
  ...
  var _num$toString$split = num.toString().split("."),
    _num$toString$split2 = _slicedToArray__default["default"](_num$toString$split, 2),
...
function decimalPlacesToFixedNum(num, decimalPlaces) {
  if (isObject.isObject(decimalPlaces)) {
...
function largeNumberToFixed(number) {
  ...
  var places = isObject.isObject(decimalPlaces) ? decimalPlaces.places : decimalPlaces;
  var enableRound = isObject.isObject(decimalPlaces) ? (_decimalPlaces$enable2 = decimalPlaces.enableRound) !== null && _decimalPlaces$enable2 !== void 0 ? _decimalPlaces$enable2 : true : true;
  if (!isString.isString(number)) return String(number);
...
//# sourceMappingURL=dep-de8595be.js.map
