...
dayjs_min.dayjs.extend(isoWeeksInYear);
dayjs_min.dayjs.extend(isLeapYear);
...
function parseToDayjs(value, format, timeOfDay, dayjsLocale) {
  if (value === "" || value === null) return dayjs_min.dayjs();
  var dateText = value;
  if (/[w|W]/g.test(format)) {
    if (!isString.isString(dateText)) {
      dateText = dayjs_min.dayjs(dateText).locale(dayjsLocale || "zh-cn").format(format);
    }
    var yearStr = dateText.split(/[-/.\s]/)[0];
    var weekStr = dateText.split(/[-/.\s]/)[1];
    var weekFormatStr = format.split(/[-/.\s]/)[1];
    var firstWeek = dayjs_min.dayjs(yearStr, "YYYY").locale(dayjsLocale || "zh-cn").startOf("year");
    if (firstWeek.day() > 4 || firstWeek.day() === 0) firstWeek = firstWeek.add(1, "week");
    var weekCounts = dayjs_min.dayjs(yearStr, "YYYY").locale(dayjsLocale || "zh-cn").isoWeeksInYear();
  ...
  if (/Q/g.test(format)) {
    ...
    var firstQuarter = dayjs_min.dayjs(_yearStr, "YYYY").startOf("year");
  ...
  var result = dayjs_min.dayjs(dateText, format).isValid() ? dayjs_min.dayjs(dateText, format) : dayjs_min.dayjs(dateText);
  if (!result.isValid()) {
...
function isValidDate(value, format) {
  if (Array.isArray(value)) {
    return value.every(function (v) {
      if (v === "") return true;
      return dayjs_min.dayjs(v, format).isValid() || dayjs_min.dayjs(v).isValid();
    });
  }
  if (value === "") return true;
  return dayjs_min.dayjs(value, format).isValid() || dayjs_min.dayjs(value).isValid();
}
...
function formatTime(value, format, timeFormat, defaultTime) {
  ...
  result = result.map(function (v, i) {
    if (v) return dayjs_min.dayjs(v, typeof v === "string" ? format : void 0).format(timeFormat);
...
function initYearMonthTime(_ref5) {
  ...
  var defaultYearMonthTime = {
    year: [dayjs_min.dayjs().year(), dayjs_min.dayjs().year()],
    month: [dayjs_min.dayjs().month(), dayjs_min.dayjs().month()],
    time: [dayjs_min.dayjs().format(timeFormat), dayjs_min.dayjs().format(timeFormat)]
  };
  if (mode === "year") {
