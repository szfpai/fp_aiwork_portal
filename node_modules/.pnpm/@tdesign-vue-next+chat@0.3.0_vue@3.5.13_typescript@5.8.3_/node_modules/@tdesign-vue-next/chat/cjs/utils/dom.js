...
function once(element, event, handler, options) {
  var handlerFn = isFunction.isFunction_1(handler) ? handler : handler.handleEvent;
...
var getAttach = function getAttach(node, triggerNode) {
  var attachNode = isFunction.isFunction_1(node) ? node(triggerNode) : node;
  if (!attachNode) {
    return document.body;
  }
  if (isString.isString_1(attachNode)) {
...
var getScrollContainer = function getScrollContainer() {
  ...
  if (isString.isString_1(container)) {
    return document.querySelector(container);
  }
  if (isFunction.isFunction_1(container)) {
    return container();
  }
  return container;
};
